...
def merged_env(env_file: Optional[Path] = None) -> dict[str, str]:
    ...
    if env_file:
        # Prefer python-dotenv when installed, but keep the stdlib parser as a
        # dependency-free fallback for locked-down environments.
        if importlib.util.find_spec("dotenv") is not None:
            dotenv = importlib.import_module("dotenv")
            dotenv_values = dotenv.dotenv_values(str(env_file))
            merged.update({key: str(value) for key, value in dotenv_values.items() if value is not None})
        else:
...
def optional_dependency_statuses() -> list[dict[str, Any]]:
    return [
        {"name": "pyodbc", "available": importlib.util.find_spec("pyodbc") is not None, "required_for": "ClientProfile SQL Server live extraction", "install": "python -m pip install -r requirements-db.txt"},
        {"name": "oracledb", "available": importlib.util.find_spec("oracledb") is not None, "required_for": "Oracle EWS live queries", "install": "python -m pip install -r requirements-db.txt"},
        {"name": "python-dotenv", "available": importlib.util.find_spec("dotenv") is not None, "required_for": "optional private .env loading", "install": "python -m pip install -r requirements-db.txt"},
    ]
...
def main() -> None:
    ...
    if args.check:
        payload = {
            ...
            "dependencies": optional_dependency_statuses(),
